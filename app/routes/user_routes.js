...
const { generateToken, requireToken } = require("../../lib/jwtAuth");
...
router.get("/sign-out", requireToken, (req, res) => {
 ...
 req.logout((err) => {
   ...
   // If logout is successful, you can clear the session and redirect, for example:
   req.session.destroy()
  })
  // Clear or invalidate the token (if using token-based authentication)
  // You may need to store tokens in a database or use a token blacklist
  // For JWT, you can simply clear the client-side token
  // res.clearCookie('your-token-cookie-name');
